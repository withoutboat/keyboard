...
1. **46 Physical Keys Layout**:
   ...
   * 6 thumb keys (3 per half):
     * Left thumbs: `Ctrl`, `Win (GUI)`, `Space`.
     * Right thumbs: `Enter` (across all layers), `Lower (Fn)`, `Raise (Nav)`.
   * Outer right keys:
     * Top: `Backspace` (standard place across all layers).
     * Middle: `' / "` (`KC_QUOT`), producing Russian letter `Э` in RU layout and `' / "` in EN layout.
     * Bottom: `Right Shift`.
...
2. **Hold / Tap Layer Switching**:
   * **Fn (Lower Layer 1 - Symbols & Russian: Х Ъ Ё Э)**:
     * **Hold**: Temporarily activates Lower layer (while held).
     * **Tap**: Locks into Layer 1 (TapDance `TG(1)`). Tap again to return to Base (`TO(0)`).
     * **Symbols**: Numbers 1-0 on top row with shifted symbols (`!`, `@`, `#`, `$`, `%`, `^`, `&`, `*`, `(`, `)`) placed directly underneath them on the home row; math symbols (`-`, `=`, `_`, `+`, `\`, `|`) and brackets/letters (`[ Х`, `] Ъ`, `' Э`) on the bottom row.
   * **Nav (Raise Layer 2 - Navigation & Media)**:
     * **Hold**: Temporarily activates Raise layer (while held).
     * **Tap**: Locks into Layer 2 (TapDance `TG(2)`). Tap again to return to Base (`TO(0)`).
     * **Navigation**: Vim arrows on right hand (`H` Left, `J` Down, `K` Up, `L` Right), and `Page Up` / `Page Down` (along with `Home`, `End`, `Insert`) on left hand.
     * **Media**: Volume and playback controls (`Mute`, `Vol-`, `Vol+`, `Prev`, `Play`, `Next`) on bottom left.
